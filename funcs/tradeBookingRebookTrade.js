 ...
 */
export function tradeBookingRebookTrade(client, rebookTradeRequestCreate, accountId, tradeId, options) {
    return new APIPromise($do(client, rebookTradeRequestCreate, accountId, tradeId, options));
}
async function $do(client, rebookTradeRequestCreate, accountId, tradeId, options) {
    ...
    const parsed = safeParse(input, (value) => operations.BookingRebookTradeRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const path = pathToFunc("/booking/v1/accounts/{account_id}/trades/{trade_id}:rebook")(pathParams);
