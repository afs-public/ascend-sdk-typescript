...
export async function assignLargeTrader(lnp_id: string): Promise<string | undefined> {
    const request = {
...
export async function createAccount(lnp_id: string): Promise<string | undefined>{
...
export async function addParty(account_id: string, legalEntityId: string): Promise<string | undefined> {
...
export async function replaceParty(account_id: string, lnp_id: string, party_id: string): Promise<string | undefined> {
  ...
  const result = await sdk.accountManagement.replaceParty(request, account_id, party_id);
  if (result?.party?.partyId) {
      return result.party.partyId
  }
  return undefined
...
export async function createTrustedContact(account_id: string): Promise<string | undefined> {
  ...
  const result = await sdk.accountManagement.createTrustedContact(request, account_id);
...
export async function createInterestedParty(account_id: string): Promise<string | undefined> {
  ...
  const result = await sdk.accountManagement.createInterestedParty(request, account_id);
...
export async function createRestrictionCode(account_id: string): Promise<string | undefined> {
  ...
  const result = await sdk.accountManagement.createRestriction(request, account_id);
...
export async function enrollAccount(account_id: string): Promise<string[] | undefined> {
  const request: components.EnrollAccountRequestCreate = {
    enrollment: {
      ...
      individualEnrollmentMetadata: {
        fdicCashSweep: components.IndividualEnrollmentMetadataCreateFdicCashSweep.FdicCashSweepDecline,
      }
    }
  }
  const result = await sdk.enrollmentsAndAgreements.enrollAccount(request, account_id);
  if (result?.enrollAccountResponse?.agreements?.[0]?.agreementId && result?.enrollAccountResponse?.agreements?.[1]?.agreementId) {
      return [result.enrollAccountResponse.agreements[0].agreementId, result.enrollAccountResponse.agreements[1].agreementId]
  }
  return undefined
}

export async function getEnrollmentToDeactivate(account_id: string): Promise<string | undefined> {
...
export async function affirmAgreement(account_id: string, enrollment_ids: string[]) {
