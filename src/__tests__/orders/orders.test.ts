...
test("Orders Orders Create Order Create Order1", async () => {
  expect(created_order_id).not.toBe(undefined);
});

test("Orders Orders Get Order Get Order1", async () => {
  if (typeof account_id !== "string") {
    throw new Error("account_id is undefined.");
  }
  if (typeof created_order_id !== "string") {
  ...
  const result = await sdk.orders.getOrder(account_id, created_order_id);
  expect(result.httpMeta.response.status).toBe(200);
});

test("Orders Orders Cancel Order Cancel Order1", async () => {
  if (typeof account_id !== "string") {
    throw new Error("account_id is undefined.");
  }
  if (typeof created_order_id !== "string") {
