...
export const fixed_subscriber_id = "01JJYZ16TVYZM6A6BDJ8RJRMTQ";
...
export async function get_delivery_id(): Promise<string | undefined> {
  const result = await sdk.subscriber.listPushSubscriptionDeliveries(fixed_subscriber_id);
  if (result?.listPushSubscriptionDeliveriesResponse?.pushSubscriptionDeliveries?.length) {
    return result?.listPushSubscriptionDeliveriesResponse?.pushSubscriptionDeliveries[0]?.deliveryId
  }
  return undefined
}
