...
import {create_subscriber_id, fixed_subscriber_id, get_delivery_id} from "./index";
...
test("Subscriber Events Update Push Subscription Update Push Subscription1", async () => {
  ...
  const result = await sdk.subscriber.updatePushSubscription(request, subscriber_id);
  expect(result.httpMeta.response.status).toBe(200);
});
...
test("Subscriber Events Get Push Subscription Event Delivery Get Push Subscription Event Delivery1", async () => {
  ...
  const result = await sdk.subscriber.getPushSubscriptionDelivery(fixed_subscriber_id, delivery_id);
  expect(result.httpMeta.response.status).toBe(200);
});
