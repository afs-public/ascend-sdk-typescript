...
import {createBankRelationship, failingMicroDeposits, correctMicroDeposits} from "./index";
...
test("Bank Relationships Transfers Get Bank Relationships Get Bank Relationships1", async () => {
  ...
  const result = await sdk.bankRelationships.getBankRelationship(account_id, bank_relationship_id);
  expect(result.httpMeta.response.status).toBe(200);
});
...
test("Bank Relationships Transfers Reissue Micro Deposits Reissue Micro Deposits1", async () => {
  ...
  const failing_micro_deposits = await failingMicroDeposits(account_id, bank_relationship_id);
  ...
  while (attempts < max_attempts) {
    attempts++;
    try {
      await sdk.bankRelationships.verifyMicroDeposits(request, account_id, bank_relationship_id);
    } catch (error) {
  ...
  const result = await sdk.bankRelationships.reissueMicroDeposits(reissue_request, account_id, bank_relationship_id);
  expect(result.httpMeta.response.status).toBe(200);
});
...
test("Bank Relationships Transfers Verify Micro Deposits Verify Micro Deposits1", async () => {
  ...
  const  correct_micro_deposits= await correctMicroDeposits(account_id, bank_relationship_id);
  ...
  const result = await sdk.bankRelationships.verifyMicroDeposits(request, account_id, bank_relationship_id);
  expect(result.httpMeta.response.status).toBe(200);
});
...
test("Bank Relationships Transfers Cancel Bank Relationships Cancel Bank Relationships1", async () => {
  ...
  const result = await sdk.bankRelationships.cancelBankRelationship(request, account_id, bank_relationship_id);
  expect(result.httpMeta.response.status).toBe(200);
});
