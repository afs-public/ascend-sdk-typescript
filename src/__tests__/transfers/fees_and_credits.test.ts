...
test("Fees and Credits Transfers Cancel Fee Cancel Fee1", async () => {
  ...
  const result = await sdk.feesAndCredits.cancelFee(request, account_id, fee_id);
  expect(result.httpMeta.response.status).toBe(200);
});
...
test("Fees and Credits Transfers Cancel Credit Cancel Credit1", async () => {
  ...
  const result = await sdk.feesAndCredits.cancelCredit(request, account_id, credit_id);
  expect(result.httpMeta.response.status).toBe(200);
});
